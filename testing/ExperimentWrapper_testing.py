...
from BenchmarkSaver import SheetSaver
    ...
    def train_a3(
        ...
        for cur_data in self.data_setup:
            for cur_n_anomaly in self.n_anomaly_samples:
                ...
                all_cb = []
                if show_metrics:
                    global anomaly_samples
                    anomaly_samples = cur_n_anomaly
                    global glob_out_path
                    glob_out_path = os.path.join(os.path.dirname(__file__), "Generated Sheets")
...
def start_SheetSavers():
    """
    Start the SheetSavers
    """
    sheetname = f"CM_{anomaly_samples}_anomalies.csv"
    global CM_Sheet
    CM_Sheet = SheetSaver(glob_out_path, sheetname, anomaly_samples)

    sheetname = f"Training_times_{anomaly_samples}_anomalies.csv"
    global Training_times_Sheet
    Training_times_Sheet = SheetSaver(glob_out_path, sheetname, anomaly_samples)
...
class Metrics(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        cm = confusion_matrix(self.validation_data[1], val_pred)
        cm_history[0].append(cm[0][0])
        cm_history[1].append(cm[0][1])
        cm_history[2].append(cm[1][0])
        cm_history[3].append(cm[1][1])
        CM_Sheet.addData(cm_history[0], f"True Negatives")
        CM_Sheet.addData(cm_history[1], f"False Positives")
        CM_Sheet.addData(cm_history[2], f"False Negatives")
        CM_Sheet.addData(cm_history[3], f"True Positives")
        CM_Sheet.save()
